...
```ruby
products = Product.search "Low fat Milk"
...
```ruby
Product.search "Low fat Milk", where: {in_stock: true}, limit: 10, offset: 50
```
...
```ruby
Product.search "Low fat Milk", facets: {store_id: {limit: 10}}
```
...
```ruby
products =
  Product.search "Low fat Milk" do |body|
