...
class BoostTest < Minitest::Test
  ...
  def test_boost
    ...
    assert_order ["Tomato C", "Tomato B", "Tomato A"], search("tomato", boost: "orders_count")
    assert_order ["Tomato C", "Tomato B", "Tomato A"], search("tomato").boost("orders_count")
  end
  ...
  def test_boost_zero
    ...
    assert_order ["Zero Boost"], search("zero", boost: "orders_count")
    assert_order ["Zero Boost"], search("zero").boost("orders_count")
  end
  ...
  def test_fields
    ...
    assert_order ["Red", "White"], search("red", fields: ["name^10", "color"])
    assert_order ["Red", "White"], search("red").fields("name^10", "color")
  end
  ...
  def test_fields_decimal
    ...
    assert_order ["Red", "White"], search("red", fields: ["name^10.5", "color"])
    assert_order ["Red", "White"], search("red").fields("name^10.5", "color")
  end
  ...
  def test_fields_word_start
    ...
    assert_order ["Red", "White"], search("red", fields: [{"name^10" => :word_start}, "color"])
    assert_order ["Red", "White"], search("red").fields({"name^10" => :word_start}, "color")
  end
  ...
  def test_fields_apostrophes
    ...
    assert_search ["Valentine's Day Special"], search("Valentines", fields: ["name^5"])
    assert_search ["Valentine's Day Special"], search("Valentine's", fields: ["name^5"])
    assert_search ["Valentine's Day Special"], search("Valentine", fields: ["name^5"])
  end
  ...
  def test_boost_by
    ...
    assert_order ["Tomato C", "Tomato B", "Tomato A"], search("tomato", boost_by: [:orders_count])
    assert_order ["Tomato C", "Tomato B", "Tomato A"], search("tomato").boost_by(:orders_count)
    assert_order ["Tomato C", "Tomato B", "Tomato A"], search("tomato", boost_by: {orders_count: {factor: 10}})
    assert_order ["Tomato C", "Tomato B", "Tomato A"], search("tomato").boost_by(orders_count: {factor: 10})
  end
  ...
  def test_boost_by_missing
    ...
    assert_order ["Tomato A", "Tomato B"], search("tomato", boost_by: {orders_count: {missing: 100}})
    assert_order ["Tomato A", "Tomato B"], search("tomato").boost_by(orders_count: {missing: 100})
  end
  ...
  def test_boost_by_boost_mode_multiply
    ...
    assert_order ["Tomato B", "Tomato A", "Tomato C"], search("tomato", boost_by: {found_rate: {boost_mode: "multiply"}})
    assert_order ["Tomato B", "Tomato A", "Tomato C"], search("tomato").boost_by(found_rate: {boost_mode: "multiply"})
  end
  ...
  def test_boost_by_indices
    ...
    assert_order ["Rexx", "Rex"], Searchkick.search("Rex", models: [Animal, Product], indices_boost: {Animal => 1, Product => 200}, fields: [:name])
  end
end
