...
class MatchTest < Minitest::Test
  ...
  def test_cross_fields
    skip if elasticsearch_below11?
    store [
      {name: "apple", color: "black"},
      {name: "carrot", color: "blue"},
      {name: "red apple", color: "pink"},
    ]
    assert_search "black apple", ["apple", "red apple"], fields: [:name, :color], match: :cross_fields
    assert_search "red or blue apple", ["apple", "carrot", "red apple"], fields: [:name, :color], match: :cross_fields, operator: 'or'
    assert_order "black carrot", ["carrot", "apple"], fields: ['name^9', :color], match: :cross_fields, operator: 'or'
    assert_order "black carrot", ["apple", "carrot"], fields: [:name, 'color^9'], match: :cross_fields, operator: 'or'
  end
